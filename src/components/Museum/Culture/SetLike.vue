  ...
  <div id="setLike">
    <Modal id="setLike" class-name="vertical-center-modal" title="设置点赞数" v-model="isShowModel" width="400">
      <Form class="mt10" ref="formValidate" :model="likeInfo" :rules="ruleValidate" :label-width="80" @submit.native.prevent>
        <FormItem label="点赞数" prop="likeNum">
          <Input v-model.trim="likeInfo.likeNum" placeholder="" number autofocus></Input>
        </FormItem>
...
import { setAntiqueLikeNum } from './Culture.service.js';
  ...
  methods: {
    handleSubmit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          const { antiqueId, likeNum } = this.likeInfo;
          const postData = {
            antiqueId,
            likeNum
          };
          setAntiqueLikeNum(postData).then(({ code }) => {
            if (+code == 10000) {
