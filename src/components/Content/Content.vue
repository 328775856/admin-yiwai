...
export default {
          ...
          render(h, params) {
            ...
            return (
              <div>
                <span
                  onClick={() =>
                    _this.$router.push({ name: 'AddArticle', params: { data } })
                  }
                  class="edit"
                >
                  编辑
                ...
                <span
                  class="edit"
                  onClick={_this.deleteArticle.bind(_this, this.row.id)}
                >
                  删除
                </span>
              </div>
        ...
        },
        {
          ...
          render(h, params) {
            if (this.row.itemType === 1) {
              return <div class="txt-2ovh">{this.row.title}</div>
            } else if (this.row.itemType === 2) {
              return <div class="txt-2ovh">{this.row.commentDto.content}</div>
          ...
          render(h, params) {
            ...
            return (
              <div>
                <span
                  onClick={() =>
                    _this.$router.push({ name: 'AddContent', params: { data } })
                  }
                  class="edit"
                >
                  编辑
                ...
                <span
                  class="edit"
                  onClick={_this.deleteHomePageChoiceness.bind(
                    _this,
                    this.row.choicenessId
                  )}
                >
                  删除
                </span>
      ...
      artShowColumns: [
        ...
        },
        {
          ...
          render(h, params) {
            ...
            const end =
              Date.parse(new Date(params.row.exhibitionEndTime)) / 1000 //结束时间转化成时间戳
            const start =
              Date.parse(new Date(params.row.exhibitionStartTime)) / 1000 // 开始时间转化成时间戳
            let statusText = '进行中'
            if (timestamp < start) {
              statusText = '未开始'
          ...
          render(h, params) {
            ...
            return (
              <div>
                <span
                  onClick={() =>
                    _this.$router.push({ name: 'AddArtShow', params: { data } })
                  }
                  class="edit"
                >
                  编辑
                ...
                <span
                  ...
                  onClick={() =>
                    _this.$router.push({
                      path: '/Comment',
                      query: { exhibitionId: this.row.id, tab: 2 }
                    })
                  }
                >
                  评论管理
                ...
                <span
                  onClick={_this.deleteExhibition.bind(_this, this.row.id)}
                  class="edit"
                >
                  删除
                </span>
  ...
  created() {
    if (this.$route.params.tab === 0 || this.$route.params.tab === '0') {
      this.index = 0
    } else if (this.$route.params.tab === 1 || this.$route.params.tab === '1') {
      this.index = 1
    } else if (this.$route.params.tab === 2 || this.$route.params.tab === '2') {
      this.index = 2
    } else {
      this.index = 0
    }

    if (this.$route.params.exhibitionName) {
      this.exhibitionName = this.$route.params.exhibitionName
    }
  },
  mounted() {
