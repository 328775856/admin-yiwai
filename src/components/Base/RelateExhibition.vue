<template>
  <div >
    <Breadcrumb separator=">" class="breadCrumb">
      ...
      <BreadcrumbItem>关联展览</BreadcrumbItem>
    </Breadcrumb>
    ...
    <!-- EyChoose begin-->
    <Modal class="eyChoose" title="关联展览" v-model="cIsShowModel" :width="800">
      <div class="mt20">
...
export default {
  ...
  methods: {
    ...
    async getDataList() {
      ...
      console.log(params)
      const { code, msg, data } = await getOrganExhibitionList(params)
      if (code == 10000) {
    ...
    async addProduct(exhibitionIds) {
      const { code, msg } = await setOrganExhibition({
        organId: this.organId,
    ...
    async delete(productId) {
      const { code, msg } = await deleteOrganExhibition({
        ids: productId
      })
      if (+code === 10000) {
        setTimeout(() => {
