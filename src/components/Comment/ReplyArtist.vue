  ...
  methods: {
    getReplyList() {
      if (!this.isLock) {
        ...
        getArtistReplyList(postData).then(({ data: { artistReplyList } }) => {
          this.isLock = false;
          if (artistReplyList && artistReplyList.length == 0) {
            this.isLoadFinish = true;
          } else {
            this.replyList = [...this.replyList, ...artistReplyList];
          }
        });
      }
    },
    ...
    deleteReply({ replyId }) {
      this.$Modal.confirm({
        ...
        content: `<p>确定要删除回复？</p>`,
        onOk: () => {
          deleteArtistReply({ replyId }).then(({ code }) => {
            if (code == '10000') {
