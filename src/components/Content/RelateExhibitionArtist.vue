<template>
    <div id="relateExhibitionArtist">
      <Breadcrumb separator=">" class="breadCrumb">
        <div>当前位置:</div>
        <BreadcrumbItem to="/Base">展览</BreadcrumbItem>
        <BreadcrumbItem>展览签约艺术家</BreadcrumbItem>
      </Breadcrumb>
      <div class="mb20">
     ...
     <!-- EyChoose begin-->
    <Modal class="eyChoose" title="关联展览艺术家" v-model="cIsShowModel" :width="800">
      <div class="mt20">
...
export default {
  ...
  data() {
    return {
      dataList: [],
      cIsShowModel: false,
      exhibitionId: Number(this.$route.query.exhibitionId),
      cSearch: '',
      loading: true,
      cData: [],
      cLoading: false,
  ...
  methods: {
    async getDataList() {
      ...
      const { code, msg, data } = await getExhibitionArtistList(params)
      if (+code === 10000) {
        this.loading = false
    ...
    async addRelationArtist(relationArtistIds) {
      const { code, msg } = await setExhibitionArtist({
        exhibitionId: this.exhibitionId,
    ...
    async delete(productId) {
      const { code, msg } = await deleteExhibitionArtist({
        ids: productId
      })
      if (+code === 10000) {
        setTimeout(() => {
